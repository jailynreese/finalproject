...
JounralSchema.statics.findByOwner = (ownerId, callback) => {
  ...
  return JournalModel.find(search).select('date').lean().exec(callback);
};

// JounralSchema.statics.findEntryByDate = (ownerId, date, callback) => {
//     const search = {
//         owner: convertID(ownerId),
//         date: date,
//     };
//     return JournalModel.find(search).select('dayRating mood entry').lean().exec(callback);
// };
